    ...
    Ownable // Inherit Ownable for admin management
{
    ...
    constructor(address _lowRisk, address _highRisk, address _admin) {
        ...
        require(_lowRisk != address(0) && _highRisk != address(0) && _admin != address(0), "Invalid address");
    ...
    function setUserStrategy(address strategy) external {
        ...
        require(strategy == lowRiskStrategy || strategy == highRiskStrategy, "Invalid strategy");
     ...
     */
    function setLowRiskStrategy(address _newLowRiskStrategy) external onlyOwner {
        require(_newLowRiskStrategy != address(0), "Invalid address");
        require(_newLowRiskStrategy != highRiskStrategy, "Strategy already set as high risk");
     ...
     */
    function setHighRiskStrategy(address _newHighRiskStrategy) external onlyOwner {
        require(_newHighRiskStrategy != address(0), "Invalid address");
        require(_newHighRiskStrategy != lowRiskStrategy, "Strategy already set as low risk");
