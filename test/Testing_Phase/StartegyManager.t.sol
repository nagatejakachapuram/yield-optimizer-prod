...
contract StrategyManagerTest is Test {
    ...
    function setUp() public {
        ...
        manager = new StrategyManager(address(mockLowRiskStrategy), address(highRiskStrategy), deployer);
        vm.stopPrank();
    }
    ...
    function test_UserCanSetLowRiskStrategyPreference() public {
        ...
        vm.stopPrank();
        assertEq(
            manager.userStrategyChoice(user1),
            address(mockLowRiskStrategy),
            "User1's strategy choice should be low risk"
        );
    }
    ...
    function test_UserCanSetHighRiskStrategyPreference() public {
        ...
        vm.stopPrank();
        assertEq(
            manager.userStrategyChoice(user2), address(highRiskStrategy), "User2's strategy choice should be high risk"
        );
    }
    ...
    function test_GetUserStrategyReturnsCorrectChoice() public {
        ...
        assertEq(
            manager.getUserStrategy(user1), address(mockLowRiskStrategy), "getUserStrategy should return user1's choice"
        );
        assertEq(
            manager.getUserStrategy(user2), address(highRiskStrategy), "getUserStrategy should return user2's choice"
        );
